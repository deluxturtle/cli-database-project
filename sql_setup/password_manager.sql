...
CREATE TABLE `has_card` (
  ...
  CONSTRAINT `fk_user_card_user` FOREIGN KEY (`username`) REFERENCES `user` (`username`) ON UPDATE CASCADE ON DELETE CASCADE,
  CONSTRAINT `fk_user_card_card` FOREIGN KEY (`card_name`) REFERENCES `card` (`card_name`) ON UPDATE CASCADE ON DELETE CASCADE
);
CREATE TABLE `has_note` (
  `username` varchar(75) NOT NULL,
  `secure_note_name` varchar(50) NOT NULL,
  PRIMARY KEY (`username`,`secure_note_name`),
  CONSTRAINT `fk_user_note_user` FOREIGN KEY (`username`) REFERENCES `user` (`username`) ON UPDATE CASCADE ON DELETE CASCADE,
  CONSTRAINT `fk_user_note_note` FOREIGN KEY (`secure_note_name`) REFERENCES `secure_note` (`secure_note_name`) ON UPDATE CASCADE ON DELETE CASCADE
);
CREATE TABLE `has_login` (
  `username` varchar(75) NOT NULL,
  `login_name` VARCHAR(50) NOT NULL,
  PRIMARY KEY (`username`,`login_name`),
  CONSTRAINT `fk_user_login_user` FOREIGN KEY (`username`) REFERENCES `user` (`username`) ON UPDATE CASCADE ON DELETE CASCADE,
  CONSTRAINT `fk_user_login_login` FOREIGN KEY (`login_name`) REFERENCES `login` (`login_name`) ON UPDATE CASCADE ON DELETE CASCADE
);
...
('Chase card', 'Alice Smith', 1111222233334444, 'VISA', 09/26/2021, 123),
('BOA card', 'Bob Bobb', 1234567812345678, 'Master Card', 09/26/2021, 567),
('Black card', 'Charlie Brown', 9999000088887777, 'AmEx', 09/26/2021, 777);
...
INSERT INTO `card` VALUES 
('Apple Card', 'Alice Smith', 1455666872222199, 'VISA', 09/26/2021, 545),
('Costco card', 'Bob Bobb', 1234567812345678, 'VISA', 09/26/2021, 999),
('Jetblue card', 'Charlie Brown', 9999000088887777, 'Master Card', 09/26/2021, 222);
